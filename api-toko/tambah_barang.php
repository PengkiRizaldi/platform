<?php
include "koneksi.php";

// Izinkan CORS jika diperlukan oleh frontend
header("Access-Control-Allow-Origin: *");
header("Content-Type: application/json; charset=UTF-8");
header("Access-Control-Allow-Headers: Content-Type, Authorization");

// =================== BAGIAN PENGUNCI API (SUDAH DIPERBAIKI) ===================
// Mengambil token secara aman tanpa bergantung pada fungsi Apache
$token_dikirim = '';

if (isset($_SERVER['HTTP_AUTHORIZATION'])) {
    $token_dikirim = $_SERVER['HTTP_AUTHORIZATION'];
} elseif (isset($_SERVER['REDIRECT_HTTP_AUTHORIZATION'])) {
    // Beberapa hosting memindahkan header ke variabel REDIRECT
    $token_dikirim = $_SERVER['REDIRECT_HTTP_AUTHORIZATION'];
} elseif (function_exists('apache_request_headers')) {
    // Alternatif terakhir jika fungsi apache ternyata tersedia
    $headers = apache_request_headers();
    $token_dikirim = isset($headers['Authorization']) ? $headers['Authorization'] : '';
}

// Cek apakah token dikirim, dan apakah token tersebut ada di tabel users
$cek_token = mysqli_query($koneksi, "SELECT * FROM users WHERE token='$token_dikirim'");

if(!$cek_token || mysqli_num_rows($cek_token) === 0 || $token_dikirim === '') {
    // JIKA TOKEN PALSU / KOSONG, HENTIKAN PROGRAM DISINI!
    http_response_code(401); // Set status HTTP menjadi 401 Unauthorized agar frontend tahu
    die(json_encode(["status" => "error", "pesan" => "Akses Ditolak! Token Invalid atau Kosong."]));
}
// ==============================================================================

// MENERIMA JSON DARI JAVASCRIPT FETCH
$json_data = file_get_contents("php://input");
$data = json_decode($json_data, true);

// Validasi jika data tidak kosong
if(isset($data['nama_barang']) && isset($data['harga'])) {
    
    // Ambil nilai dari array JSON dan amankan dari SQL Injection
    $nama = mysqli_real_escape_string($koneksi, $data['nama_barang']);
    $harga = mysqli_real_escape_string($koneksi, $data['harga']);

    // Query Insert ke Database
    $query = "INSERT INTO barang (nama_barang, harga) VALUES ('$nama', '$harga')";
    
    if(mysqli_query($koneksi, $query)) {
        echo json_encode(["status" => "success", "pesan" => "Data barang berhasil disimpan!"]);
    } else {
        http_response_code(500);
        echo json_encode(["status" => "error", "pesan" => "Gagal menyimpan ke database: " . mysqli_error($koneksi)]);
    }

} else {
    http_response_code(400); // Bad Request
    echo json_encode(["status" => "error", "pesan" => "Data tidak lengkap!"]);
}
?>
